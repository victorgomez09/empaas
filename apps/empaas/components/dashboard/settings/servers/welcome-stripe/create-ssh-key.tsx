...
export const CreateSSHKey = () => {
	...
	const cloudSSHKey = data?.find(
		(sshKey) => sshKey.name === "empaas-cloud-ssh-key",
	);
	...
	useEffect(() => {
		const createKey = async () => {
			...
			try {
				...
				await mutateAsync({
					name: "empaas-cloud-ssh-key",
					description: "Used on Empaas Cloud",
							...
							<div className="flex flex-col gap-4 text-sm text-muted-foreground">
								...
								{selectedOption === "provider" && (
									<div className="flex flex-col gap-2 w-full border rounded-lg p-4">
										...
										<Link
											href="https://docs.empaas.com/docs/core/multi-server/instructions#requirements"
											target="_blank"

...
const notificationBaseSchema = z.object({
	...
	empaasRestart: z.boolean().default(false),
		...
		if (notification) {
			if (notification.notificationType === "slack") {
				form.reset({
					...
					empaasRestart: notification.empaasRestart,
								...
								{!isCloud && (
									<FormField
										...
										render={({ field }) => (
											<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm gap-2">
												<div className="space-y-0.5">
													<FormLabel>Empaas Restart</FormLabel>
													<FormDescription>
														Trigger the action when empaas is restarted.
													</FormDescription>
												</div>
												<FormControl>
													<Switch

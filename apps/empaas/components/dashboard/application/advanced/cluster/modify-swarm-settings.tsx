...
const StopGraceForm = ({ form }: { form: any }) => {
	return (
		<FormField
			control={form.control}
...
export const AddSwarmSettings = ({ id, type }: Props) => {
	...
	const convertFormToApiFormat = (formData: SwarmSettingsForm) => {
		...
		result.stopGracePeriodSwarm = formData.stopGracePeriodSwarm ?? null;
	...
	const convertApiToFormFormat = (apiData: any): SwarmSettingsForm => {
		return {
			...
			stopGracePeriodSwarm: apiData.stopGracePeriodSwarm,
		};
	};
					...
					>
						<Tabs defaultValue="health-check" className="w-full">
							<TabsList className="grid w-full grid-cols-4 lg:grid-cols-8">
								...
								<TabsTrigger value="stop-grace-period">
									Stop Grace Period
								</TabsTrigger>
							</TabsList>
							...
							<TabsContent value="stop-grace-period" className="space-y-4">
								<div className="space-y-4 p-4 border rounded-lg">
									<div className="space-y-1">
										<FormLabel className="text-base font-medium">
											Stop Grace Period
										...
										<FormDescription>
											specifies how long Compose must wait when attempting to
											stop a container if it doesn't handle SIGTERM (or
											whichever stop signal has been specified with
											stop_signal), before sending SIGKILL.
										</FormDescription>
									</div>
									<StopGraceForm form={form} />
								</div>
							</TabsContent>
						</Tabs>

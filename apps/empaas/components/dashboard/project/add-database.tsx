...
export const AddDatabase = ({ environmentId, projectName }: Props) => {
	...
	// Cloud: show only if there are remote servers (no empaas option)
	// Self-hosted: show only if there are remote servers (empaas is default, hide if no remote servers)
	...
	const onSubmit = async (data: AddDatabase) => {
		...
		const commonParams = {
			...
			serverId: data.serverId === "empaas" ? undefined : data.serverId,
			environmentId,
		...
		if (data.type === "libsql") {
			promise = libsqlMutation.mutateAsync({
				...
				serverId: data.serverId === "empaas" ? null : data.serverId,
			});
										...
										render={({ field }) => (
											<FormItem>
												...
												<Select
													onValueChange={field.onChange}
													defaultValue={
														field.value || (!isCloud ? "empaas" : undefined)
													}
												>
													<SelectTrigger>
														<SelectValue
															placeholder={
																!isCloud ? "empaas" : "Select a Server"
															}
														/>
													</SelectTrigger>
													<SelectContent>

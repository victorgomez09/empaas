...
cat /etc/empaas/ssh/auto_generated-empaas-local.pub >> $HOME/.ssh/authorized_keys && \\
echo "✓ Empaas SSH key added successfully. Reopen the terminal in Empaas to reconnect."
# ----------------------------------------`;

const COMMAND_TO_GRANT_PERMISSION_ACCESS = `
# ----------------------------------------
sudo chown -R $USER:$USER /etc/empaas/ssh
# ----------------------------------------
`;

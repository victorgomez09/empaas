...
export const settingsRouter = createTRPCRouter({
	reloadServer: adminProcedure.mutation(async () => {
		...
		await reloadDockerResource("empaas");
		return true;
	}),
	cleanRedis: adminProcedure.mutation(async () => {
		...
		const { stdout: containerId } = await execAsync(
			`docker ps --filter "name=empaas-redis" --filter "status=running" -q | head -n 1`,
		);
	...
	reloadRedis: adminProcedure.mutation(async () => {
		...
		await reloadDockerResource("empaas-redis");
	...
	reloadTraefik: adminProcedure
		...
		.mutation(async ({ input }) => {
			try {
				await reloadDockerResource("empaas-traefik", input?.serverId);
			} catch (err) {
				console.error(err);
			}
	...
	readWebServerTraefikConfig: adminProcedure.query(() => {
		...
		const traefikConfig = readConfig("empaas");
		return traefikConfig;
	}),
	updateWebServerTraefikConfig: adminProcedure
		...
		.mutation(async ({ input }) => {
			...
			writeConfig("empaas", input.traefikConfig);
			return true;
		}),
	...
	updateServer: adminProcedure.mutation(async () => {
		...
		// This causes restart of empaas, thus it will not finish executing properly, so don't await it
	...
	getEmpaasVersion: protectedProcedure.query(() => {
		return packageInfo.version;
	}),
	getReleaseTag: protectedProcedure.query(() => {
		return getEmpaasImageTag();
	}),
	...
	getOpenApiDocument: protectedProcedure.query(
		async ({ ctx }): Promise<unknown> => {
			...
			openApiDocument.info = {
				title: "Empaas API",
				description: "Endpoints for empaas",
				version: "1.0.0",
			};
	...
	toggleRequests: protectedProcedure
		...
		.mutation(async ({ input }) => {
			...
			if (input.enable) {
				const config = {
					accessLog: {
						filePath: "/etc/empaas/traefik/dynamic/access.log",
						format: "json",
						bufferingSize: 100,
						filters: {
							retryAttempts: true,
	...
	getEmpaasCloudIps: adminProcedure.query(async () => {
		if (!IS_CLOUD) {
			return [];
		}
		const ips = process.env.Empaas_CLOUD_IPS?.split(",");
		return ips;
	}),
});

...
export const notificationRouter = createTRPCRouter({
	...
	testTelegramConnection: adminProcedure
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, "Hi, From Empaas 👋");
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", "Hi, From Empaas 👋"),
					color: 0xf3f7f4,
				});
	...
	receiveNotification: publicProcedure
		.input(
			z.object({
				ServerType: z.enum(["Empaas", "Remote"]).default("Empaas"),

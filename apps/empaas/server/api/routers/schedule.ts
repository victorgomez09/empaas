import { IS_CLOUD, removeScheduleJob, scheduleJob } from "@empaas/server";
import { db } from "@empaas/server/db";
import { deployments } from "@empaas/server/db/schema/deployment";
import {
	createScheduleSchema,
	schedules,
	updateScheduleSchema,
} from "@empaas/server/db/schema/schedule";
import { runCommand } from "@empaas/server/index";
import {
	createSchedule,
	deleteSchedule,
	findScheduleById,
	updateSchedule,
} from "@empaas/server/services/schedule";
	...
	list: protectedProcedure
		...
		.query(async ({ input }) => {
			const where = {
				...
				"empaas-server": eq(schedules.userId, input.id),
			};

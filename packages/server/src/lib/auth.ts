		...
		user: {
			create: {
				before: async (_user, context) => {
					if (!IS_CLOUD) {
						const xEmpaasToken =
							context?.request?.headers?.get("x-empaas-token");
						if (xEmpaasToken) {
							const user = await getUserByToken(xEmpaasToken);
							if (!user) {
			...
			async sendInvitationEmail(data, _request) {
				if (IS_CLOUD) {
					...
					await sendEmail({
						...
						text: `
					<p>You are invited to join ${data.organization.name} on Empaas. Click the link to accept the invitation: <a href="${inviteLink}">Accept Invitation</a></p>
					`,
					});
				}
			},

...
import { findAdmin } from "@empaas/server/services/admin";
import { updateUser } from "@empaas/server/services/user";
	...
	try {
		...
		scheduleJob(LOG_CLEANUP_JOB_NAME, cronExpression, async () => {
			try {
				...
				await execAsync("docker exec empaas-traefik kill -USR1 1");
			} catch (error) {

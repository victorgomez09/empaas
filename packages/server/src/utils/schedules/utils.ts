...
import type { Schedule } from "@empaas/server/db/schema/schedule";
import {
	createDeploymentSchedule,
	updateDeployment,
	updateDeploymentStatus,
} from "@empaas/server/services/deployment";
import { findScheduleById } from "@empaas/server/services/schedule";
...
export const runCommand = async (scheduleId: string) => {
	...
	} else if (scheduleType === "empaas-server") {
		try {
			const writeStream = createWriteStream(deployment.logPath, { flags: "a" });

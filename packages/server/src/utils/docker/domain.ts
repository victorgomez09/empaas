...
import type { Compose } from "@empaas/server/services/compose";
...
export const loadDockerCompose = async (
	...
	if (existsSync(path)) {
		...
		return parse(yamlStr) as ComposeSpecification;
	}
	return null;
};
...
export const loadDockerComposeRemote = async (
	...
	try {
		...
		return parse(stdout) as ComposeSpecification;
	} catch {
		return null;
	}
};
...
export const addDomainToCompose = async (
	...
	for (const domain of domains) {
		...
		if (Array.isArray(labels)) {
			...
			if (!compose.isolatedDeployment) {
				if (compose.composeType === "docker-compose") {
					if (!labels.includes("traefik.docker.network=empaas-network")) {
						labels.unshift("traefik.docker.network=empaas-network");
					}
				} else {
					// Stack Case
					if (!labels.includes("traefik.swarm.network=empaas-network")) {
						labels.unshift("traefik.swarm.network=empaas-network");
					}
				}
			}
		}
		...
		if (!compose.isolatedDeployment) {
			// Add the empaas-network to the service
			result.services[serviceName].networks = addEmpaasNetworkToService(
				result.services[serviceName].networks,
			);
		}
	}

	// Add empaas-network to the root of the compose file
	if (!compose.isolatedDeployment) {
		result.networks = addEmpaasNetworkToRoot(result.networks);
	}
...
export const addEmpaasNetworkToService = (
	networkService: DefinitionsService["networks"],
) => {
	let networks = networkService;
	const network = "empaas-network";
	if (!networks) {
		networks = [];
	}
...
export const addEmpaasNetworkToRoot = (

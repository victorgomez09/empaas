...
import { IS_CLOUD, paths } from "@empaas/server/constants";
import type { Destination } from "@empaas/server/services/destination";
...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "empaas-restore-"));
		...
		try {
			...
			const { stdout: postgresContainer } = await execAsync(
				`docker ps --filter "name=empaas-postgres" --filter "status=running" -q | head -n 1`,
			);

			if (!postgresContainer) {
				throw new Error("Empaas Postgres container not found");
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U empaas postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'empaas' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U empaas postgres -c "DROP DATABASE IF EXISTS empaas;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U empaas postgres -c "CREATE DATABASE empaas;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U empaas -d empaas /tmp/database.sql`,
			);

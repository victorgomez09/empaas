...
import { member } from "@empaas/server/db/schema";
import type { BackupSchedule } from "@empaas/server/services/backup";
import { getAllServers } from "@empaas/server/services/server";
...
export const keepLatestNBackups = async (
	...
	try {
		...
		// --include "*.sql.gz" or "*.zip" ensures nothing else other than the empaas backup files are touched by rclone

...
import type { BackupSchedule } from "@empaas/server/services/backup";
import {
	createDeploymentBackup,
	updateDeploymentStatus,
} from "@empaas/server/services/deployment";
import { findDestinationById } from "@empaas/server/services/destination";
...
export const runWebServerBackup = async (backup: BackupSchedule) => {
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "empaas-backup-"));
		...
		try {
			...
			if (!containerId) {
				writeStream.write("Empaas postgres container not found❌\n");
				writeStream.end();
				throw new Error("Empaas postgres container not found");
			}

			writeStream.write(`Empaas postgres container ID: ${containerId}\n`);

			const postgresContainerId = containerId.trim();

			// First dump the database inside the container
			const dumpCommand = `docker exec ${postgresContainerId} pg_dump -v -Fc -U empaas -d empaas -f /tmp/database.sql`;

...
import { getEmpaasImageTag } from "../services/settings";
...
export const setupMonitoring = async (serverId: string) => {
	...
	const containerName = "empaas-monitoring";
	let imageName = "empaas/monitoring:latest";

	if (
		(getEmpaasImageTag() !== "latest" ||
			process.env.NODE_ENV === "development") &&
		!IS_CLOUD
	) {
		imageName = "empaas/monitoring:canary";
	}
	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				"/proc:/host/proc:ro",
				"/etc/empaas/monitoring/monitoring.db:/app/monitoring.db",
			],
			NetworkMode: "host",
		},
		ExposedPorts: {
	...
	try {
		await execAsyncRemote(
			serverId,
			"mkdir -p /etc/empaas/monitoring && touch /etc/empaas/monitoring/monitoring.db",
		);
		if (serverId) {

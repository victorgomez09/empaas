...
import { findServerById } from "@empaas/server/services/server";
import {
...
const setupMainDirectory = () => `
	# Check if the /etc/empaas directory exists
	if [ -d /etc/empaas ]; then
		echo "/etc/empaas already exists ✅"
	else
		# Create the /etc/empaas directory
		mkdir -p /etc/empaas
		chmod 777 /etc/empaas

		echo "Directory /etc/empaas created ✅"
	fi
`;
...
const setupNetwork = () => `
	# Check if the empaas-network already exists
	if docker network ls | grep -q 'empaas-network'; then
		echo "Network empaas-network already exists ✅"
	else
		# Create the empaas-network if it doesn't exist
		if docker network create --driver overlay --attachable empaas-network; then
			echo "Network created ✅"
		else
			echo "Failed to create empaas-network ❌" >&2
			exit 1
		fi
	fi
`;
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        ...
        echo "   Please note that Empaas does not support Docker installed via snap."
...
const createTraefikConfig = () => {
	...
	const command = `
	if [ -f "/etc/empaas/traefik/dynamic/acme.json" ]; then
		chmod 600 "/etc/empaas/traefik/dynamic/acme.json"
	fi
	if [ -f "/etc/empaas/traefik/traefik.yml" ]; then
		echo "Traefik config already exists ✅"
	else
		echo "${config}" > /etc/empaas/traefik/traefik.yml
	fi
	`;
...
const createDefaultMiddlewares = () => {
	...
	const command = `
	if [ -f "/etc/empaas/traefik/dynamic/middlewares.yml" ]; then
		echo "Middlewares config already exists ✅"
	else
		echo "${config}" > /etc/empaas/traefik/dynamic/middlewares.yml
	fi
	`;
	return command;
};
...
export const createTraefikInstance = () => {
	const command = `
		...
		if docker service inspect empaas-traefik > /dev/null 2>&1; then
			echo "Migrating Traefik to Standalone..."
			docker service rm empaas-traefik
			sleep 8
			echo "Traefik migrated to Standalone ✅"
		fi

		if docker inspect empaas-traefik > /dev/null 2>&1; then
			echo "Traefik already exists ✅"
		else
			# Create the empaas-traefik container
			TRAEFIK_VERSION=${TRAEFIK_VERSION}
			docker run -d \
				--name empaas-traefik \
				--restart always \
				-v /etc/empaas/traefik/traefik.yml:/etc/traefik/traefik.yml \
				-v /etc/empaas/traefik/dynamic:/etc/empaas/traefik/dynamic \
			...
			docker network connect empaas-network empaas-traefik;

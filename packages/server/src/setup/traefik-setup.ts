...
export const initializeStandaloneTraefik = async ({
	...
	const containerName = "empaas-traefik";
	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				`${DYNAMIC_TRAEFIK_PATH}:/etc/empaas/traefik/dynamic`,
			...
			ContainerSpec: {
				...
				Mounts: [
					...
					},
					{
						...
						Target: "/etc/empaas/traefik/dynamic",
					},
					{
						Type: "bind",
...
export const createDefaultServerTraefikConfig = () => {
	...
	const configFilePath = path.join(DYNAMIC_TRAEFIK_PATH, "empaas.yml");

...
/** Returns current Empaas docker image tag or `latest` by default. */
export const getEmpaasImageTag = () => {
	return process.env.RELEASE_TAG || "latest";
};

export const getEmpaasImage = () => {
	return `empaas/empaas:${getEmpaasImageTag()}`;
};

export const pullLatestRelease = async () => {
	const stream = await docker.pull(getEmpaasImage());
...
/** Returns Empaas docker service image digest */
export const getServiceImageDigest = async () => {
	const { stdout } = await execAsync(
		"docker service inspect empaas --format '{{.Spec.TaskTemplate.ContainerSpec.Image}}'",
	);
...
export const getUpdateData = async (): Promise<IUpdateData> => {
	...
	try {
		...
		// https://docs.empaas.com/docs/core/manual-installation
		return DEFAULT_UPDATE_DATA;
	}

	const baseUrl = "https://hub.docker.com/v2/repositories/empaas/empaas/tags";
	...
	const imageTag = getEmpaasImageTag();

...
} from "@empaas/server/utils/builders/compose";
import { randomizeSpecificationFile } from "@empaas/server/utils/docker/compose";
import {
	cloneCompose,
	cloneComposeRemote,
	loadDockerCompose,
	loadDockerComposeRemote,
} from "@empaas/server/utils/docker/domain";
import type { ComposeSpecification } from "@empaas/server/utils/docker/types";
...
import {
	execAsync,
	execAsyncRemote,
} from "@empaas/server/utils/process/execAsync";
import {
...
} from "@empaas/server/utils/providers/raw";
...
export const deployCompose = async ({
	...
	const buildLink = `${await getEmpaasUrl()}/dashboard/project/${
...
export const removeCompose = async (
	...
	try {
		...
		if (compose.composeType === "stack") {
			const command = `
			docker network disconnect ${compose.appName} empaas-traefik;

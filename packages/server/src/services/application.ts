import { docker } from "@empaas/server/constants";
...
import { getAdvancedStats } from "@empaas/server/monitoring/utils";
import {
	buildApplication,
	getBuildCommand,
	mechanizeDockerContainer,
} from "@empaas/server/utils/builders";
import { sendBuildErrorNotifications } from "@empaas/server/utils/notifications/build-error";
import { sendBuildSuccessNotifications } from "@empaas/server/utils/notifications/build-success";
import { execAsyncRemote } from "@empaas/server/utils/process/execAsync";
import {
	cloneBitbucketRepository,
	getBitbucketCloneCommand,
} from "@empaas/server/utils/providers/bitbucket";
import {
	buildDocker,
	buildRemoteDocker,
} from "@empaas/server/utils/providers/docker";
import {
	cloneGitRepository,
	getCustomGitCloneCommand,
} from "@empaas/server/utils/providers/git";
import {
	cloneGiteaRepository,
	getGiteaCloneCommand,
} from "@empaas/server/utils/providers/gitea";
import {
	cloneGithubRepository,
	getGithubCloneCommand,
} from "@empaas/server/utils/providers/github";
import {
	cloneGitlabRepository,
	getGitlabCloneCommand,
} from "@empaas/server/utils/providers/gitlab";
import { createTraefikConfig } from "@empaas/server/utils/traefik/application";
...
export const deployApplication = async ({
	...
	const buildLink = `${await getEmpaasUrl()}/dashboard/project/${application.environment.projectId}/environment/${application.environmentId}/services/application/${application.applicationId}?tab=deployments`;
...
export const deployPreviewApplication = async ({
	...
	try {
		...
		await updateIssueComment({
			...issueParams,
			body: `### Empaas Preview Deployment\n\n${buildingComment}`,
		});
		application.appName = previewDeployment.appName;
		application.env = `${application.previewEnv}\nEmpaas_DEPLOY_URL=${previewDeployment?.domain?.host}`;
